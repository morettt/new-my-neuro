...
这一步需要部署项目，为了快速测试。可以直接用云端API 这样就省去了下载各种模型的操作。（因为之前说了，后端代码几乎不用改。所以可以偷懒用我提供的云端API）
...
#### 直接双击live-2d文件夹下面的：一键打包QT.bat  （这个一键打包QT.bat用的是new-my-neuro的虚拟环境。所以记得部署好这个虚拟环境，安装下载好pip install -r requirements.txt 再来双击bat）
